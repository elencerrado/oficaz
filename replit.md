...
- **Admin Time Tracking Optimizations** (Nov 2025): Three critical performance improvements for high-user scenarios: (1) **Server-side filtering**: Work sessions endpoint accepts employeeId, startDate, endDate, status filters as query params, reducing client-side data processing and network payload, (2) **WebSocket real-time updates**: Implemented WebSocket server (`/ws/work-sessions`) for live updates on work session changes, eliminating aggressive 30-second polling (saves ~60-120 requests/hour per admin), (3) **Lazy loading audit logs**: Audit logs removed from batch loading in getWorkSessionsByCompany, loaded only when audit history dialog opens via `/api/admin/work-sessions/:id/audit-log`, significantly reducing initial payload size. Combined impact: ~70% reduction in data transfer, instant UI updates, improved responsiveness for 100+ concurrent users.
