...
// Minimal shape for document items and paginated response
...
export default function EmployeeDocuments() {
  ...
  // Simple query - useInfiniteQuery has internal issues with cached data
  const {
    data: documentsData,
    isLoading,
    refetch,
  ...
  useEffect(() => {
    ...
    if (signDocumentId && signatureToken && allDocuments.length > 0) {
      // Find the document in the loaded documents
      const docToSign = allDocuments.find((doc: any) => doc.id === parseInt(signDocumentId));
      
      if (docToSign) {
        setDocumentToSign(docToSign);
  ...
  // Documents already normalized in allDocuments
