                  ...
                  {stats.recentMovements.slice(0, 5).map((movement: any) => (
                    <div key={movement.id} className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <span className="text-sm dark:text-white font-medium">{movement.movementNumber}</span>
                        <div className="w-24 flex-shrink-0">
                          <Badge variant="outline" className="dark:border-gray-600 dark:text-gray-300 whitespace-nowrap">{getMovementLabel(movement)}</Badge>
                        </div>
                      </div>
...
function ProductsTab({ searchTerm, setSearchTerm }: { searchTerm: string; setSearchTerm: (s: string) => void }) {
  ...
  const { data: infiniteData, isLoading, hasNextPage, isFetchingNextPage, fetchNextPage } = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 0 }) => {
      ...
      const response = await fetch(`/api/inventory/products?${params}`, { headers: getAuthHeaders() });
      if (!response.ok) throw new Error('Failed to fetch products');
      const data = await response.json();
      return data.items || data;
    },
    getNextPageParam: (lastPage, allPages) => {
      // Check if lastPage has the new paginated format
      if (lastPage && typeof lastPage === 'object' && 'hasMore' in lastPage) {
        return lastPage.hasMore ? allPages.length * 50 : undefined;
      }
      // Fallback for old format (array of items)
      return undefined;
    },
    initialPageParam: 0,
    staleTime: 60_000,
  ...
  const allProducts = useMemo(
    () => (infiniteData?.pages || []).flatMap(page => (Array.isArray(page) ? page : page?.items || [])),
    [infiniteData]
  );
  ...
  }, [allProducts, searchTerm, stockByProduct, inStockOnly]);

  const displayedProducts = useMemo(() => {
    return filteredProducts.slice(0, displayedCount);
  }, [filteredProducts, displayedCount]);

  const hasMoreToDisplay = displayedCount < filteredProducts.length || (hasNextPage ?? false);

  const loadMoreProducts = useCallback(() => {
    if (displayedCount < filteredProducts.length) {
      setDisplayedCount(prev => Math.min(prev + ITEMS_PER_LOAD, filteredProducts.length + ITEMS_PER_LOAD));
    } else if (hasNextPage && !isFetchingNextPage) {
      fetchNextPage();
      setDisplayedCount(prev => prev + ITEMS_PER_LOAD);
    }
  }, [displayedCount, filteredProducts.length, hasNextPage, isFetchingNextPage, fetchNextPage, ITEMS_PER_LOAD]);
...
function MovementsTab() {
  ...
  const { data: infiniteData, isLoading, hasNextPage, isFetchingNextPage, fetchNextPage } = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 0 }) => {
      ...
      params.append('limit', '50');
      params.append('offset', pageParam.toString());
      const response = await fetch(`/api/inventory/movements?${params}`, { headers: getAuthHeaders() });
  ...
  const displayedMovements = useMemo(
    () => allMovements.slice(0, displayedCount),
    [allMovements, displayedCount]
  );

  const hasMoreToDisplay = displayedCount < allMovements.length || (hasNextPage ?? false);

  const loadMoreMovements = useCallback(() => {
    if (displayedCount < allMovements.length) {
      setDisplayedCount(prev => Math.min(prev + ITEMS_PER_LOAD, allMovements.length + ITEMS_PER_LOAD));
    } else if (hasNextPage && !isFetchingNextPage) {
      fetchNextPage();
      setDisplayedCount(prev => prev + ITEMS_PER_LOAD);
    }
  }, [displayedCount, allMovements.length, hasNextPage, isFetchingNextPage, fetchNextPage, ITEMS_PER_LOAD]);
      ...
      {isDialogOpen ? (
        ...
        <div className="space-y-3">
          {displayedMovements.map(movement => (
            <Card 
              key={movement.id} 
              className="hover:shadow-md transition-shadow dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden cursor-pointer"
              role="button"
              onClick={() => {
                if (movement.status === 'posted') {
                  // Si está confirmado, abrir en modo lectura
                  openViewDialog(movement);
                } else {
                  // Si está en borrador, abrir para editar
            ...
            >
              <CardContent className="p-0">
                <div className="flex items-stretch">
                  {/* MÓVIL: Contenido principal compacto */}
                  <div className="flex-1 min-w-0 p-3 md:hidden">
                    {/* Tipo de movimiento pequeño */}
                    <div className="mb-2">
                    ...
                    {/* Línea 1: Número de albarán + Total */}
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <span className="font-mono text-sm font-medium dark:text-white">{movement.movementNumber}</span>
                      <span className="text-base font-bold dark:text-white flex-shrink-0">
                        {parseFloat(movement.total).toFixed(2)} €
                      </span>
                    </div>

                    {/* Línea 2: Fecha + Botón eliminar */}
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        {new Date(movement.movementDate).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: '2-digit' })}
                      </span>
                      <Button 
                        variant="ghost"
                        size="icon"
                        className="h-6 w-6 p-0 text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:bg-red-900/30"
                        onClick={(e) => {
                          e.stopPropagation();
                  ...
                  {/* DESKTOP: Contenido principal normal */}
                  <div className="hidden md:flex flex-1 min-w-0 p-4 items-center gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-sm md:text-base text-gray-900 dark:text-gray-100 truncate">
                        {getMovementTypeLabel(movement)} - {movement.movementNumber}
                      </p>
                      <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">
                        {new Date(movement.movementDate).toLocaleDateString('es-ES', { weekday: 'short', day: '2-digit', month: 'short' })}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      <Button 
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 p-0 text-gray-500 hover:text-red-500 dark:text-gray-500 dark:hover:bg-red-900/30 dark:hover:text-red-400"
                        onClick={(e) => {
                          e.stopPropagation();
                          setDeleteConfirm({ open: true, movement });
                        }}
                  ...
                  {/* Estado: Barra en móvil (w-1), sección en desktop (w-[110px]) */}
                  {/* Móvil */}
                  <div className={`md:hidden w-1 flex-shrink-0 ${
                  ...
                  {/* Desktop */}
                  <div className={`hidden md:flex md:w-[110px] items-center justify-center flex-shrink-0 px-3 ${
                    movement.status === 'draft' 
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader className="pb-4 border-b dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                ...
                {viewDialog.movement && (
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {new Date(viewDialog.movement.movementDate).toLocaleDateString('es-ES', { 
                      weekday: 'long', 
                      year: 'numeric', 
                      month: 'long', 
                      day: 'numeric' 
              ...
              {viewDialog.movement && (
                <div className={`px-4 py-2 rounded-full text-sm font-medium ${statusLabels[viewDialog.movement.status]?.color || 'bg-gray-100 dark:bg-gray-700'}`}>
          ...
          {viewDialog.loading ? (
            ...
            <div className="space-y-6 py-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {viewDialog.movement.relatedPartyName && (
                    <div className="col-span-2 space-y-1">
                      <p className="text-xs text-gray-500 dark:text-gray-400">Cliente/Proveedor</p>
              ...
              {/* Almacenes */}
              {(viewDialog.movement.sourceWarehouse || viewDialog.movement.destinationWarehouse) && (
                <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 space-y-3">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-3">
                    Almacenes
                  </h3>
              ...
              {viewDialog.movement.lines && viewDialog.movement.lines.length > 0 && (
                <div className="space-y-3">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    Productos ({viewDialog.movement.lines.length})
                  </h3>
                  <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                                <span className="flex items-center gap-1">
                                  ...
                                  Cantidad: {parseFloat(line.quantity)}
                                </span>
                                {parseFloat(line.unitPrice) > 0 && (
                                  <span>Precio: {parseFloat(line.unitPrice).toFixed(2)} €</span>
                                )}
                              </div>
                            </div>
                            {parseFloat(line.unitPrice) > 0 && (
                    ...
                    {parseFloat(viewDialog.movement.total) > 0 && (
                      <div className="bg-gray-50 dark:bg-gray-900/50 px-4 py-3 border-t border-gray-200 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-lg font-bold text-gray-900 dark:text-white">
                            {parseFloat(viewDialog.movement.total).toFixed(2)} €
                          </span>
                        </div>
                      </div>
                    )}
              ...
              {viewDialog.movement.notes && (
                <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 border border-amber-200 dark:border-amber-800">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <h3 className="text-xs font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-400 mb-1">
                        Notas
                      </h3>
          ...
          <DialogFooter className="border-t dark:border-gray-700 pt-4">
            <Button 
              variant="outline" 
              onClick={() => setViewDialog({ open: false, movement: null, loading: false })}
              className="px-6"
            >
              Cerrar
            </Button>
            <Button 
              variant="outline"
              onClick={() => downloadPdf(viewDialog.movement?.id, viewDialog.movement?.movementNumber)} 
              disabled={!viewDialog.movement}
              className="px-6"
            >
              <Download className="h-4 w-4 mr-2" />
              Descargar PDF
            </Button>
            {viewDialog.movement?.status === 'posted' && (
              <Button 
                className="bg-orange-600 hover:bg-orange-700 px-6"
                onClick={() => {
                  setRevertConfirm({ open: true, movement: viewDialog.movement });
                  setViewDialog({ open: false, movement: null, loading: false });
                }}
              >
