...
export default function EmployeeTimeTracking() {
  ...
  // Get break periods for timeline visualization
  ...
  // Timeline mobile-first compact rendering
  const renderMobileTimeline = (session: WorkSession) => {
    const sessionBreaks = Array.isArray(breakPeriods) 
      ? breakPeriods.filter((bp: any) => bp.workSessionId === session.id)
      : [];
    
    const sessionDate = new Date(session.clockIn);
    const totalHours = session.clockOut ? calculateSessionHours(session) : 0;
    
    // Combine all events and sort chronologically
    const allEvents: Array<{
      time: Date;
      type: 'entrada' | 'salida' | 'break_start' | 'break_end';
      id: string;
      breakId?: number;
    }> = [];
    ...
    if (session.clockOut) {
      allEvents.push({
        time: new Date(session.clockOut),
        type: 'salida',
    ...
    // Add break events
    sessionBreaks.forEach((bp: any) => {
      allEvents.push({
        time: new Date(bp.breakStart),
        type: 'break_start',
        id: `break_start_${bp.id}`,
        breakId: bp.id
      });
      
      if (bp.breakEnd) {
        allEvents.push({
          time: new Date(bp.breakEnd),
          type: 'break_end',
          id: `break_end_${bp.id}`,
          breakId: bp.id
        });
    ...
    return (
      <div className="bg-white/5 rounded-xl p-4 mb-4 border border-white/10">
        {/* Header: Fecha y Total de Horas */}
        <div className="flex justify-between items-center mb-4">
          <div className="text-white font-medium text-sm">
            {formatDayDate(sessionDate)}
          </div>
          <div className="text-blue-400 font-bold text-sm">
            {session.clockOut ? formatTotalHours(totalHours) : 'En curso'}
          </div>
        </div>

        {/* Timeline compacto */}
        <div className="relative">
          {/* Línea azul horizontal principal */}
          <div className="absolute top-3 left-0 w-full h-0.5 bg-blue-400/30"></div>
          
          {/* Eventos cronológicos */}
          <div className="relative flex justify-between items-center h-6">
            {allEvents.map((event, index) => {
              const position = allEvents.length > 1 
                ? (index / (allEvents.length - 1)) * 100 
                : 50;

              const eventTime = formatTime(event.time.toISOString());
              
              // Punto del evento
              const pointColor = {
                'entrada': 'bg-green-500 border-green-400',
                'salida': 'bg-red-500 border-red-400', 
                'break_start': 'bg-orange-500 border-orange-400',
                'break_end': 'bg-blue-500 border-blue-400'
              }[event.type];
              ...
              return (
                <div
                  ...
                  style={{ left: `${position}%`, transform: 'translateX(-50%)', top: '0px' }}
                >
                  {/* Punto del evento */}
                  <div className={`w-3 h-3 rounded-full border-2 border-white ${pointColor} shadow-lg relative z-10`}></div>
                  
                  {/* Tooltip visible en mobile */}
        ...
        {/* Información adicional de descansos */}
        {sessionBreaks.length > 0 && (
          <div className="mt-4 pt-3 border-t border-white/10">
            <div className="text-xs text-white/70 mb-2">Descansos del día:</div>
            <div className="flex flex-wrap gap-2">
              {sessionBreaks.map((bp: any) => {
                const breakDuration = bp.breakEnd 
                  ? Math.round(differenceInMinutes(new Date(bp.breakEnd), new Date(bp.breakStart)))
                  : null;
                
                return (
                  <div key={bp.id} className="text-xs bg-white/10 rounded px-2 py-1">
                    {bp.status === 'active' ? (
                      <span className="text-orange-400">
                        ⏱️ En progreso ({formatTime(bp.breakStart)})
                      </span>
                    ) : (
                      <span className="text-blue-400">
                        🛑 {formatTime(bp.breakStart)} - {formatTime(bp.breakEnd)} ({breakDuration}m)
                      </span>
                    )}
                  </div>
                );
  ...
  return (
    <div className="min-h-screen bg-employee-gradient text-white flex flex-col page-scroll">
      ...
      <div 
        ...
        onTouchEnd={onTouchEnd}
      >
        {/* Content será reemplazado por timeline móvil */}
