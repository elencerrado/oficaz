                        ...
                        <TableBody>
                          {[{ id: 'new', company: '', phone: '', location: '', tags: [] }, ...filteredProspects].map((prospect: any) => (
                            <TableRow key={prospect.id} className={`border-white/20 hover:bg-white/5 ${prospect.id === 'new' ? 'bg-purple-500/10' : ''}`}>
                              ...
                              <TableCell
                                className={`text-white ${editingCell?.id === prospect.id && editingCell?.field === 'tags' ? 'overflow-visible' : 'overflow-hidden'} ${prospect.id !== 'new' ? 'cursor-pointer hover:bg-white/10' : 'opacity-40'}`}
                                onDoubleClick={() => {
                                  if (prospect.id !== 'new') {
